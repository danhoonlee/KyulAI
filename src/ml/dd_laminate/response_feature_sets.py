...
def response_feature_row(case: str, theta1: float, theta2: float, feature_set: str = "theta") -> list[float]:
    theta = response_theta_feature_row(case, theta1, theta2)
    if feature_set == "theta":
        return theta
    if feature_set == "theta_physics":
        physics = extended_physics_feature_vector(case, theta1, theta2).tolist()
        return [*theta, *physics]
    if feature_set == "theta_physics_v2":
        physics = compact_physics_feature_vector(case, theta1, theta2).tolist()
        return [*theta, *physics]
    if feature_set == "theta_physics_nn_v2":
        physics = nn_friendly_physics_feature_vector(case, theta1, theta2).tolist()
        return [*theta, *physics]
...
def response_feature_matrix(records, feature_set: str = "theta") -> tuple[np.ndarray, list[str]]:
    rows = [
        response_feature_row(record.case, float(record.theta1), float(record.theta2), feature_set)
...
def prediction_feature_matrix(
    ...
    record = ResponseFeatureRecord(case=case, theta1=theta1, theta2=theta2)
