...
def candidate_models(random_state: int) -> dict[str, Pipeline]:
    return {
        ...
        "svc_rbf": Pipeline([
            ...
            ("model", SVC(C=5.0, gamma="scale", class_weight="balanced", probability=True, random_state=random_state)),
        ]),
...
def split_iter(cv_mode: str, splits: int, random_state: int, x, y, groups):
    if cv_mode == "sample":
        yield from StratifiedKFold(n_splits=splits, shuffle=True, random_state=random_state).split(x, y)
    elif cv_mode == "grouped":
        yield from StratifiedGroupKFold(n_splits=splits, shuffle=True, random_state=random_state).split(x, y, groups)
    else:
...
def write_report(out: Path, data_dir: str, best: str, primary: dict, secondary: dict, conflicts: list[dict], rows: list[dict]) -> None:
    ...
    if conflicts:
        ...
        for c in conflicts:
            sample_text = "; ".join(f"{s['case']}/{s['test_id']}=Type{s['label']}" for s in c["samples"])
    ...
    for name, r in sorted(secondary.items(), key=lambda item: item[1]["mean_macro_f1"], reverse=True):
        lines.append(f"| {name} | {r['mean_accuracy']:.4f} ± {r['std_accuracy']:.4f} | {r['mean_macro_f1']:.4f} ± {r['std_macro_f1']:.4f} | {r['mean_weighted_f1']:.4f} |")
    lines.extend([
        "",
...
def train_theta_classifier(data_dir: str, output_dir: str, splits: int = 5, random_state: int = 42):
    ...
    write_rows(out / "theta_training_rows.csv", rows, ["case", "test_id", "theta1", "theta2", "label"])
    ...
    if conflict_rows:
        write_rows(out / "theta_label_conflicts.csv", conflict_rows, ["case", "test_id", "theta1", "theta2", "label"])

    best, primary = cross_validate(candidate_models(random_state), x, y, groups, splits, random_state, "sample")
    _, secondary = cross_validate(candidate_models(random_state), x, y, groups, splits, random_state, "grouped")
    ...
    grouped_best = max(secondary, key=lambda n: (secondary[n]['mean_macro_f1'], secondary[n]['mean_accuracy']))
