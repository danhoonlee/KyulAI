...
FEATURE_REGISTRY: list[FeatureSpec] = [
    ...
    FeatureSpec("injection_pressure_MPa",   lambda r: (
        _pp(r).injection_pressure_Pa / 1e6 if _pp(r).injection_pressure_Pa else None
    ), scale=10.0),
    FeatureSpec("compaction_pressure_MPa",  lambda r: (
        _pp(r).compaction_pressure_Pa / 1e6 if _pp(r).compaction_pressure_Pa else None
    ), scale=0.7),
    FeatureSpec("blank_holder_force_kN",    lambda r: (
        _pp(r).blank_holder_force_N / 1e3 if _pp(r).blank_holder_force_N else None
    ), scale=100.0),
    ...
    FeatureSpec("band_width_mm",            lambda r: (
        _pp(r).band_width_m * 1e3 if _pp(r).band_width_m else None
    ), scale=10.0),
    FeatureSpec("num_plies",                lambda r: (
        float(_pp(r).num_plies) if _pp(r).num_plies else None
    ), scale=20.0),
    ...
    FeatureSpec("youngs_modulus_GPa",       lambda r: (
        _el(r).youngs_modulus_Pa / 1e9 if _el(r).youngs_modulus_Pa else None
    ), scale=70.0),
    FeatureSpec("poisson_ratio",            lambda r: _el(r).poisson_ratio,            scale=0.3),
    FeatureSpec("shear_modulus_GPa",        lambda r: (
        _el(r).shear_modulus_Pa / 1e9 if _el(r).shear_modulus_Pa else None
    ), scale=30.0),

    # ── Material: elastic (transversely isotropic) ────────────────────────────
    FeatureSpec("E1_GPa",                   lambda r: (
        _el(r).E1_Pa / 1e9 if _el(r).E1_Pa else None
    ), scale=150.0),
    FeatureSpec("E2_GPa",                   lambda r: (
        _el(r).E2_Pa / 1e9 if _el(r).E2_Pa else None
    ), scale=10.0),
    FeatureSpec("G12_GPa",                  lambda r: (
        _el(r).G12_Pa / 1e9 if _el(r).G12_Pa else None
    ), scale=5.0),
    FeatureSpec("G23_GPa",                  lambda r: (
        _el(r).G23_Pa / 1e9 if _el(r).G23_Pa else None
    ), scale=4.0),
    FeatureSpec("nu12",                     lambda r: _el(r).nu12,                     scale=0.3),
    FeatureSpec("nu23",                     lambda r: _el(r).nu23,                     scale=0.4),

    # ── Material: thermal ─────────────────────────────────────────────────────
    FeatureSpec("thermal_conductivity_W_mK", lambda r: (
    ...
    FeatureSpec("cte_per_K_1e6",            lambda r: (
        _th(r).cte_per_K * 1e6
    ...
    # ── Material: fiber ───────────────────────────────────────────────────────
    FeatureSpec("fiber_diameter_um",        lambda r: (
        _fib(r).fiber_diameter_m * 1e6 if _fib(r).fiber_diameter_m else None
    ), scale=7.0),
    FeatureSpec("fiber_density_kg_m3",      lambda r: _fib(r).fiber_density_kg_per_m3, scale=1800.0),
    FeatureSpec("fiber_modulus_GPa",        lambda r: (
        _fib(r).fiber_modulus_Pa / 1e9 if _fib(r).fiber_modulus_Pa else None
    ), scale=230.0),
    FeatureSpec("fiber_strength_GPa",       lambda r: (
        _fib(r).fiber_strength_Pa / 1e9 if _fib(r).fiber_strength_Pa else None
    ), scale=4.0),
    FeatureSpec("fiber_aspect_ratio",       lambda r: _fib(r).fiber_aspect_ratio,     scale=100.0),

    # ── Loading conditions ────────────────────────────────────────────────────
    FeatureSpec("applied_force_x_N",        lambda r: (
        _lc(r).applied_force_N[0] if _lc(r).applied_force_N else None
    ), scale=10000.0),
    FeatureSpec("applied_force_y_N",        lambda r: (
        _lc(r).applied_force_N[1] if _lc(r).applied_force_N else None
    ), scale=10000.0),
    FeatureSpec("applied_force_z_N",        lambda r: (
        _lc(r).applied_force_N[2] if _lc(r).applied_force_N else None
    ), scale=10000.0),
    FeatureSpec("applied_pressure_MPa",     lambda r: (
        _lc(r).applied_pressure_Pa / 1e6 if _lc(r).applied_pressure_Pa else None
    ), scale=10.0),
    ...
    FeatureSpec("forming_speed_mm_s",       lambda r: (
        _proc(r).forming_speed_m_per_s * 1000 if _proc(r).forming_speed_m_per_s else None
    ), scale=10.0),
    FeatureSpec("winding_tension_N",        lambda r: _proc(r).winding_tension_N,     scale=50.0),

    # ── Geometry summary statistics ───────────────────────────────────────────
    FeatureSpec("num_nodes_log10",          lambda r: (
        float(np.log10(r.geometry.num_nodes)) if r.geometry.num_nodes > 0 else None
    ), scale=5.0),
    FeatureSpec("num_elements_log10",       lambda r: (
    ...
    FeatureSpec("tool_moldex3d",  lambda r: 1.0 if r.metadata.source_tool.value == "moldex3d" else 0.0,  scale=1.0),
    FeatureSpec("tool_aniform",   lambda r: 1.0 if r.metadata.source_tool.value == "aniform" else 0.0,   scale=1.0),
    FeatureSpec("tool_digimat",   lambda r: 1.0 if r.metadata.source_tool.value == "digimat" else 0.0,   scale=1.0),
    FeatureSpec("tool_abaqus",    lambda r: 1.0 if r.metadata.source_tool.value == "abaqus" else 0.0,    scale=1.0),
    FeatureSpec("tool_simutence", lambda r: 1.0 if r.metadata.source_tool.value == "simutence" else 0.0, scale=1.0),
    FeatureSpec("tool_cadfil",    lambda r: 1.0 if r.metadata.source_tool.value == "cadfil" else 0.0,    scale=1.0),

    # ── Simulation type one-hot ───────────────────────────────────────────────
    FeatureSpec("simtype_structural",      lambda r: 1.0 if r.metadata.simulation_type.value == "structural"      else 0.0, scale=1.0),
    FeatureSpec("simtype_flow",            lambda r: 1.0 if r.metadata.simulation_type.value == "flow"            else 0.0, scale=1.0),
    FeatureSpec("simtype_thermal",         lambda r: 1.0 if r.metadata.simulation_type.value == "thermal"         else 0.0, scale=1.0),
    FeatureSpec("simtype_forming",         lambda r: 1.0 if r.metadata.simulation_type.value == "forming"         else 0.0, scale=1.0),
    FeatureSpec("simtype_curing",          lambda r: 1.0 if r.metadata.simulation_type.value == "curing"          else 0.0, scale=1.0),
    FeatureSpec("simtype_winding",         lambda r: 1.0 if r.metadata.simulation_type.value == "winding"         else 0.0, scale=1.0),
    FeatureSpec("simtype_micromechanics",  lambda r: 1.0 if r.metadata.simulation_type.value == "micromechanics"  else 0.0, scale=1.0),
]
...
class FeatureExtractor:
    ...
    def extract_batch(
        self, records: list[UnifiedCAERecord]
    ...
    def to_tensor(
        self, record: UnifiedCAERecord, device: Any = "cpu"
    ) -> tuple[Any, Any]:
