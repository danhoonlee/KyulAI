...
def _metric_row(
    ...
    pred_pressure = y_curve_pred * np.maximum(y_scalars_pred[:, 1:2], 1e-9)
    return {
        ...
        "curve_norm_rmse": float(np.sqrt(np.mean((y_curve_pred - y_curve_true) ** 2))),
...
def cross_validate(
    ...
    for name, base_model in models.items():
        ...
        for fold, (train_idx, val_idx) in enumerate(split_iter(cv_mode, splits, random_state, x, groups), start=1):
            ...
            row = _metric_row(y_scalars[val_idx], y_curve[val_idx], pred_scalars, pred_curve)
            row["fold"] = fold
