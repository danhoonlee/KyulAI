...
def run_epoch(model, loader, optimizer, device, train: bool) -> tuple[float, np.ndarray, np.ndarray]:
...
def train_one_fold(dataset, train_idx, val_idx, args, y_mean, y_std) -> dict[str, float]:
    ...
    for epoch in range(1, args.epochs + 1):
        run_epoch(model, train_loader, optimizer, args.device_torch, train=True)
        val_loss, _, _ = run_epoch(model, val_loader, optimizer, args.device_torch, train=False)
    ...
    _, pred_norm, true_norm = run_epoch(model, val_loader, optimizer, args.device_torch, train=False)
...
def train_final(dataset, args) -> SimpleInjectionGointRegressor:
    ...
    for _ in range(args.final_epochs):
        run_epoch(model, loader, optimizer, args.device_torch, train=True)
    return model
