...
def run_epoch(model, loader, optimizer, device, train: bool, args, grid_t: torch.Tensor):
...
def train_one_fold(dataset, train_idx, val_idx, args, device, scalar_mean, scalar_std, grid_np):
    ...
    row = metric_row(val_out, scalar_mean, scalar_std, grid_np)
...
def train_final(dataset, args, device):
    ...
    model = make_model(args, dataset.x.shape[1], dataset.y_curve.shape[1], device)
    grid_t = torch.linspace(0.0, 1.0, dataset.y_curve.shape[1], dtype=torch.float32, device=device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    for _ in range(args.final_epochs):
        run_epoch(model, loader, optimizer, device, train=True, args=args, grid_t=grid_t)
    return model
