...
const MODEL_LABELS_KO = {
  ...
  "GointMLP NN": "GointMLP NN",
  "DeepONet NN": "DeepONet NN",
  "ExtraTrees histogram": "ExtraTrees Histogram",
  ...
  "Sprue pressure - RandomForest + PCA": "Sprue Pressure - RandomForest + PCA",
  "Sprue pressure - Classical ML + PCA": "Sprue Pressure - ExtraTrees + PCA",
  "Sprue pressure - GointMLP-style NN": "Sprue Pressure - GointMLP NN",
  "Sprue pressure - DeepONet operator NN": "Sprue Pressure - DeepONet NN",
  "Filling pressure - ExtraTrees histogram": "Filling Pressure - ExtraTrees Histogram",
  "Filling pressure - Classical ML histogram": "Filling Pressure - ExtraTrees Histogram",
  "Filling pressure - GointMLP-style NN": "Filling Pressure - GointMLP NN",
  "Filling pressure - DeepONet histogram NN": "Filling Pressure - DeepONet NN",
};
const NOTE_LABELS_KO = {
  "Current model is trained on 360 Moldex3D cases covering G01-G42.": "현재 모델은 G01-G42 범위의 Moldex3D 해석 360건으로 학습되었습니다.",
  ...
  "The DeepONet model is an operator-learning research model for smoother curve behavior on user-edited DOE conditions.": "DeepONet 모델은 사용자 입력 DOE 조건에서 더 자연스러운 곡선 형태를 보기 위한 operator-learning 연구 모델입니다.",
};
...
const comparisonSprueTitle = document.querySelector("#comparison-sprue-title");
const comparisonFillingTitle = document.querySelector("#comparison-filling-title");
...
function formPayload() {
  ...
  const payload = {
    ...
    filling_model: data.get("filling_model"),
...
function clearComparisonOutput() {
  ...
  if (comparisonSprueTitle) {
    comparisonSprueTitle.textContent = IS_KO ? "Sprue Pressure 오차" : "Sprue Pressure Error";
  }
  if (comparisonFillingTitle) {
    comparisonFillingTitle.textContent = IS_KO ? "Filling Pressure 오차" : "Filling Pressure Error";
  }
...
function renderComparisonResult(data) {
  ...
  if (comparisonSprueTitle && latestPredictionData?.model_label) {
    comparisonSprueTitle.textContent = `${IS_KO ? "Sprue Pressure 오차" : "Sprue Pressure Error"} (${localizeModelLabel(latestPredictionData.model_label)})`;
  }
  if (comparisonFillingTitle && latestPredictionData?.filling_model_label) {
    comparisonFillingTitle.textContent = `${IS_KO ? "Filling Pressure 오차" : "Filling Pressure Error"} (${localizeModelLabel(latestPredictionData.filling_model_label)})`;
  }
...
function buildResultReportCanvas({ paginateForPdf = false } = {}) {
  ...
  ctx.fillStyle = "#0076bd";
  ctx.font = "900 28px system-ui, sans-serif";
  ctx.fillText("Injection AI", 64, 72);
  ctx.fillStyle = "#132236";
...
function renderFillingPressure(summary, inputs = {}, predictedSummary = null) {
  ...
  if (fillingSummaryEyebrow) {
    fillingSummaryEyebrow.textContent = predictedSummary ? "Generated Preview" : (isManualInput ? "Generated Preview" : "Moldex3D Export");
  }
...
function renderResult(data) {
  ...
  if (sprueModelLabel) {
    sprueModelLabel.textContent = localizeModelLabel(data.model_label);
  }
  const sprueLabel = localizeModelLabel(data.model_label);
  const fillingLabel = localizeModelLabel(data.filling_model_label);
  modelLabel.textContent = `${IS_KO ? "Sprue" : "Sprue"}: ${sprueLabel} / ${IS_KO ? "Filling" : "Filling"}: ${fillingLabel}`;
