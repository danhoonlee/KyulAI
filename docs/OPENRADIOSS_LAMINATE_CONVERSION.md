...
The scripts import `src.` directly and the package is not installed into the
serving venv, so run them from the repo root with `PYTHONPATH=.`:

```bash
PYTHONPATH=. .venv/bin/python scripts/inp2rad_laminate.py \
