...
def predict_tree(path: Path, x: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    pred_scalars, pred_curves = _postprocess(pred_scalars, pred_curves, grid, smooth=False)
...
def predict_deep(
    ...
    pred_scalars, pred_curves = _postprocess(pred_scalars, pred_curves, grid, smooth=True)
...
def label_confidences(data_dir: Path, records: list[DDRecord]) -> np.ndarray:
    ...
    values = {(row["case"], row["Test_ID"]): float(row["type_label_confidence"]) for row in rows}
    return np.asarray([values[(record.case, record.test_id)] for record in records], dtype=float)
...
def main() -> None:
    ...
    model_predictions = {
        "Geometry Tree canonical v2": predict_tree(args.tree_model, x),
        "Geometry GointMLP canonical v2": predict_deep(args.goint_model, x, device, args.batch_size),
        "Hybrid Student canonical v2": predict_deep(args.hybrid_model, x, device, args.batch_size),
    }
